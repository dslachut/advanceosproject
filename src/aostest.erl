...
test() ->
    ...
    Node0 = spawn(worker,startNode,[F1,1,[],0,self()]),
    spawn(worker,startNode,[F2,2,[#neighbor{pid=Node0,age=0}],0,self()]),
    spawn(worker,startNode,[F4,4,[#neighbor{pid=Node0,age=0}],0,self()]),
    spawn(worker,startNode,[F1,1,[#neighbor{pid=Node0,age=0}],0,self()]),
    spawn(worker,startNode,[F2,2,[#neighbor{pid=Node0,age=0}],0,self()]),
    spawn(worker,startNode,[F3,3,[#neighbor{pid=Node0,age=0}],0,self()]),
    spawn(worker,startNode,[F4,4,[#neighbor{pid=Node0,age=0}],0,self()]),
    spawn(worker,startNode,[F5,5,[#neighbor{pid=Node0,age=0}],0,self()]),
    spawn(worker,startNode,[F6,6,[#neighbor{pid=Node0,age=0}],0,self()]),
    timer:sleep(10000),
    Node0 ! hello,
    Node0 ! neighbors,
    Node0 ! longestWord,
    receive 
        Msg1 -> io:format("Longword: ~p~n",[Msg1])
    after 
